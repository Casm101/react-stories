...
export const ReactStory = ({
  ...
  const mousedownId = useRef<number>(null);
  ...
  useEffect(() => {
    ...
    if (preloadedAssets > 0) {
      setImagesPreload(
        stories.slice(currentStory, currentStory + preloadedAssets).map(story => {
    ...
    const handleVideoLoaded = () => {
      if (stories[currentStory].type === 'video' && videoRef.current)
    ...
    setStoryTimers(
      Array.from({ length: numStories }).map((_, index) => (
        <ProgressBar
          ...
          customDuration={
            stories[index].type === 'video' ? videoDuration : stories[index].storyDuration
          }
          key={index}
        />
      ))
